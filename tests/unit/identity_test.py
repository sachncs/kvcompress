...
to ``method="pass"``, ``"fp16"``, ``"bf16"``, and ``"fp8"``.
"""
...
def test_pass_preserves_shape(k_v: tuple[torch.Tensor, torch.Tensor]) -> None:
    k, v = k_v
    c = Pass()
...
def test_pass_dtype_cast_to_factor(k_v: tuple[torch.Tensor, torch.Tensor]) -> None:
    """Default dtype is fp16; stored K/V values are fp16 even when input is fp32."""
    k, v = k_v
    c = Pass()
    kp, _ = c.compress(k, v)
...
def test_pass_dtype_override(k_v: tuple[torch.Tensor, torch.Tensor]) -> None:
    """dtype=bf16 stores values in bf16."""
    k, v = k_v
    c = Pass(dtype=torch.bfloat16)
    kp, _ = c.compress(k, v)
    assert kp.data["value"].dtype == torch.bfloat16


def test_pass_dtype_fp32_roundtrip(k_v: tuple[torch.Tensor, torch.Tensor]) -> None:
    """dtype=fp32 makes the round-trip bit-exact."""
    k, v = k_v
    c = Pass(dtype=torch.float32)
    kp, vp = c.compress(k, v)
...
def test_pass_dtype_restore_on_decompress(k_v: tuple[torch.Tensor, torch.Tensor]) -> None:
    """decompress returns the original input dtype, not the storage dtype."""
    k = k_v[0].to(torch.float32)
    c = Pass(dtype=torch.float16)
    kp, _ = c.compress(k, k)
    k_hat, _ = c.restore(kp, kp)
    assert k_hat.dtype == torch.float32


def test_pass_kv_separation(k_v: tuple[torch.Tensor, torch.Tensor]) -> None:
...
def test_pass_stats_correct(k_v: tuple[torch.Tensor, torch.Tensor]) -> None:
    """``bytes_compressed`` reflects the dtype's element size."""
    k, v = k_v
    c = Pass(dtype=torch.float16)
    kp, _ = c.compress(k, v)
