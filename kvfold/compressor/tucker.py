...
class TuckerFactors:
    ...
    def r_token(self) -> int:
        """Token-mode rank (``rT``) after truncation.

        Equals ``u_token.shape[1]``. Stored as ``min(int(r_token), T)``.
        """
        return int(self.u_token.shape[1])

    @property
    def r_feature(self) -> int:
        """Feature-mode rank (``rd``) after truncation.

        Equals ``u_feature.shape[1]``. Stored as ``min(int(r_feature), dh)``.
        """
