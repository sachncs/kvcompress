...
## "Exactly one of `target_memory` or `ratio` must be provided."
...
## "Invalid `cache_implementation` (`kvfold`)."

`kvfold` automatically maps whatever you pass to `cache_implementation`
to `"dynamic"` under the hood, since Hugging Face's strict allow-list does
not include `"kvfold"`. This message means the mapping failed for some
...
Currently supported: `jolt`, `flash`, `low`, `pass`, `int2`, `int4`,
`int8`, `fp8`, `fp16`, `bf16`.
...
2. Try `method="jolt"` instead of `flash` to rule out the randomized
   SVD.
...
1. Use `flash` instead of `jolt`.
2. Install the `triton` extra: `pip install "kvfold[triton]"`. (Note:
